...
def get_me(current_user=Depends(get_current_active_user)):
    ...
    Depends(get_current_active_user) automatically:
    1. Reads the Authorization header from the request
    2. Decodes the JWT token
    3. Looks up the user in DB
    4. Passes the User object here as 'current_user'
    
    If no valid token → 401 error returned automatically.
    The frontend calls this to check "who am I?" on page load.
    """
    return current_user
